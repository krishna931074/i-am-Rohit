...
async def restart_handler(_, m):
    await m.reply_text("**Stopped**馃殾", True)
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        try:
            ...
            await m.reply_text("Invalid file input.馃ゲ")
            os.remove(x)
            return
    else:
        content = input.text
    ...
    try:
        await bot.send_message(-1002276233363, f"**鈥ile name** - `{file_name}`({raw_text0})\n**鈥otal Links Found In TXT** - `{len(links)}`\n**鈥tarts from** - `{raw_text}`\n**鈥esolution** - `{res}`({raw_text2})\n**鈥aption** - `{raw_text3}`\n**鈥humbnail** - `{thumb}`")
        for i in range(count - 1, len(links)):
            ...
            try:
                ...
                cc = f'**[馃摴]Video_ID : {str(count).zfill(3)}.**\n\n**Video Name :** {name1} ({res}) {CR}.mkv\n\n**Batch Name :** {b_name}\n\n**Downloaded By : {CR}**'
                cc1 = f'**[馃搧]File_ID : {str(count).zfill(3)}.**\n\n**File Name :**{name1} {CR}.pdf\n\n**Batch Name :**{b_name}\n\n**Downloaded By : {CR}**'
                ...
                logging.error(e)
                await m.reply_text(f"**Failed To Download 鉂�**\n**Name** - {name}\n**Link** - `{url}`")
                time.sleep(3)
                continue
    except Exception as e:
